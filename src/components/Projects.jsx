...
const categories = [
  {
    ...
    gradient: "linear-gradient(135deg, rgba(99,102,241,0.08) 0%, rgba(99,102,241,0.15) 50%, rgba(79,70,229,0.1) 100%)",
  ...
  },
  {
    ...
    gradient: "linear-gradient(135deg, rgba(244,63,94,0.08) 0%, rgba(244,63,94,0.15) 50%, rgba(225,29,72,0.1) 100%)",
  ...
  },
  {
    ...
    gradient: "linear-gradient(135deg, rgba(20,184,166,0.08) 0%, rgba(20,184,166,0.15) 50%, rgba(13,148,136,0.1) 100%)",
  ...
  },
  {
    ...
    gradient: "linear-gradient(135deg, rgba(14,165,233,0.08) 0%, rgba(14,165,233,0.15) 50%, rgba(2,132,199,0.1) 100%)",
  ...
  },
  {
    ...
    gradient: "linear-gradient(135deg, rgba(245,158,11,0.08) 0%, rgba(245,158,11,0.15) 50%, rgba(217,119,6,0.1) 100%)",
...
const ProjectImageCarousel = ({ images, img, name }) => {
  ...
  if (!images || images.length === 0) {
    return (
      <div className="relative h-[220px] md:h-[400px] overflow-hidden bg-slate-100">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-white/80 via-transparent to-transparent opacity-80"></div>
      </div>
    );
  }

  return (
    <div className="relative h-[220px] md:h-[400px] overflow-hidden group/carousel bg-slate-100">
      ...
      <button
        ...
        className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/70 backdrop-blur-sm border border-slate-200 flex items-center justify-center text-slate-500 opacity-0 group-hover/carousel:opacity-100 hover:bg-indigo-50 hover:text-indigo-500 transition-all"
      >
      ...
      <button
        ...
        className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/70 backdrop-blur-sm border border-slate-200 flex items-center justify-center text-slate-500 opacity-0 group-hover/carousel:opacity-100 hover:bg-indigo-50 hover:text-indigo-500 transition-all"
      >
      ...
      <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
        {images.map((_, i) => (
          <button
            ...
            className={`w-1.5 h-1.5 rounded-full transition-all ${i === currentIndex ? "bg-indigo-500 w-3" : "bg-slate-300 hover:bg-slate-400"}`}
          />
        ))}
      </div>
    </div>
...
export default function Projects() {
  ...
  return (
    <section id="projects" className="relative py-24 overflow-hidden" style={{ background: "rgba(240,242,247,0.95)" }}>
      <div className="px-5 md:px-12">
        <h2 className="text-center text-xs font-semibold tracking-[.2em] uppercase text-slate-400 mb-2 reveal">Projects</h2>
        <p className="text-center text-[clamp(1.6rem,3vw,2.2rem)] font-display font-bold mb-4 reveal reveal-delay-1 text-slate-800">
          Some of my <span className="text-indigo-500">Work</span>
        </p>
        <p className="text-center text-sm text-slate-400 mb-12 reveal reveal-delay-2 max-w-lg mx-auto">
      ...
      {showPopup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm" onClick={() => setShowPopup(false)}>
          <div className="bg-white border border-slate-200 rounded-2xl p-8 max-w-sm mx-4 text-center shadow-2xl" style={{ animation: "fadeUp .3s ease both" }}>
            <div className="w-14 h-14 rounded-full bg-indigo-50 flex items-center justify-center mx-auto mb-4">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#6366f1" strokeWidth="2"><circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" /></svg>
            </div>
            <h4 className="text-lg font-bold text-slate-800 mb-2">Not Live Yet</h4>
            <p className="text-sm text-slate-500">This project is currently not deployed. Check back soon for the live version!</p>
          </div>
        </div>
      )}
      ...
      {!activeCategory && (
        <div className="relative max-w-[1200px] mx-auto px-5 md:px-14">
          <button onClick={() => { scrollTo(-1); pauseAutoplay(); }} className="absolute left-0 md:left-1 top-1/2 -translate-y-1/2 z-10 w-9 h-9 md:w-10 md:h-10 rounded-full bg-white/80 border border-slate-200 flex items-center justify-center text-slate-500 hover:bg-indigo-50 hover:border-indigo-200 hover:text-indigo-500 transition-all shadow-sm">
          ...
          <button onClick={() => { scrollTo(1); pauseAutoplay(); }} className="absolute right-0 md:right-1 top-1/2 -translate-y-1/2 z-10 w-9 h-9 md:w-10 md:h-10 rounded-full bg-white/80 border border-slate-200 flex items-center justify-center text-slate-500 hover:bg-indigo-50 hover:border-indigo-200 hover:text-indigo-500 transition-all shadow-sm">
          ...
          >
            {categories.map((cat) => (
              <div
                ...
                className="snap-start shrink-0 w-[240px] md:w-[270px] rounded-2xl overflow-hidden cursor-pointer group transition-all duration-500 hover:-translate-y-3 hover:shadow-[0_20px_50px_rgba(99,102,241,.1)]"
                style={{ border: "1.5px solid rgba(226,232,240,0.8)", background: "rgba(255,255,255,0.95)" }}
              >
                <div className="p-4 md:p-5 pb-3">
                  <h3 className="text-[0.9rem] md:text-[0.95rem] font-bold text-slate-800 mb-1.5 group-hover:text-indigo-500 transition-colors duration-300">{cat.name}</h3>
                  <p className="text-[0.68rem] md:text-[0.72rem] text-slate-400 leading-relaxed">{cat.desc}</p>
                </div>
                <div className="relative h-[140px] md:h-[170px] mx-3 mb-3 rounded-xl overflow-hidden">
                  ...
                  <div className="absolute bottom-3 left-3 w-8 h-8 md:w-9 md:h-9 rounded-full bg-white/70 backdrop-blur-sm border border-slate-200 flex items-center justify-center text-slate-500 group-hover:bg-indigo-50 group-hover:border-indigo-200 group-hover:text-indigo-500 transition-all duration-300">
                  ...
                  <div className="absolute top-3 right-3 px-2 py-0.5 rounded-full bg-white/70 backdrop-blur-sm text-[0.58rem] text-slate-600 font-semibold">
      ...
      {activeCategory && (
        <div className="px-4 md:px-12 max-w-[1100px] mx-auto" style={{ animation: "fadeUp .5s ease both" }}>
          <button onClick={goBack} className="flex items-center gap-2 text-sm text-slate-500 hover:text-indigo-500 transition-colors mb-8 group">
          ...
          <div className="mb-8 md:mb-10">
            <h3 className="font-display text-xl md:text-2xl font-bold text-slate-800 mb-2">
              {categories.find((c) => c.id === activeCategory)?.name}
            </h3>
            <div className="w-16 h-[3px] rounded-full bg-indigo-500"></div>
          </div>
          ...
          <div className="flex flex-col gap-6 md:gap-8">
            {(projects[activeCategory] || []).map((p, i) => (
              <div
                key={p.name}
                className="group rounded-2xl overflow-hidden border border-slate-200/80 hover:border-indigo-200 transition-all duration-500"
                style={{ background: "rgba(255,255,255,0.9)", animation: `fadeUp .5s ${i * 0.15}s ease both` }}
              >
                {/* Image Carousel */}
                ...
                <div className="p-5 md:p-7">
                  <h4 className="text-base md:text-lg font-bold text-slate-800 mb-2 md:mb-3 group-hover:text-indigo-500 transition-colors duration-300">{p.name}</h4>
                  <p className="text-xs md:text-sm text-slate-500 leading-relaxed mb-4 md:mb-5">{p.desc}</p>
                  <div className="flex flex-wrap gap-1.5 md:gap-2 mb-5 md:mb-6">
                    {p.tech.map((t) => (
                      <span key={t} className="px-2 md:px-2.5 py-0.5 md:py-1 rounded-full border border-slate-200 bg-slate-50 text-[0.62rem] md:text-[0.68rem] text-slate-500 font-medium">{t}</span>
                    ))}
                  </div>
                  <div className="flex gap-3">
                    {p.github && (
                      <a href={p.github} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-3 md:px-4 py-2 rounded-lg bg-slate-50 border border-slate-200 text-xs font-semibold text-slate-600 hover:border-indigo-200 hover:text-indigo-500 transition-all">
                    ...
                    <button onClick={() => handleLiveClick(p.live)} className="flex items-center gap-2 px-3 md:px-4 py-2 rounded-lg bg-indigo-50 border border-indigo-200 text-xs font-semibold text-indigo-500 hover:bg-indigo-100 transition-all">
