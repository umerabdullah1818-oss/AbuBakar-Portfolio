...
export default function Contact() {
  ...
  return (
    <section id="contact" className="px-12 py-24 max-md:px-6" style={{ background: "rgba(10,10,10,0.95)" }}>
      <h2 className="text-center text-xs font-semibold tracking-[.2em] uppercase text-gray-500 mb-2 reveal">Get In Touch</h2>
      <p className="text-center text-[clamp(1.6rem,3vw,2.2rem)] font-display font-bold mb-12 reveal reveal-delay-1 text-gray-100">
        Let's work together
      </p>
      ...
      <div className="max-w-[600px] mx-auto flex flex-col gap-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div className="reveal reveal-delay-2">
            <label className="block text-xs font-semibold tracking-wider uppercase text-gray-500 mb-2">Name</label>
            <input id="cName" type="text" placeholder="Your full name" className="form-input" />
          </div>
          <div className="reveal reveal-delay-2">
            <label className="block text-xs font-semibold tracking-wider uppercase text-gray-500 mb-2">Email</label>
        ...
        <div className="reveal reveal-delay-3" ref={dropRef}>
          <label className="block text-xs font-semibold tracking-wider uppercase text-gray-500 mb-2">Service</label>
          <div className="relative">
            <button
              type="button"
              onClick={() => setDropOpen(!dropOpen)}
              className="w-full text-left bg-[#111] border-[1.5px] border-gray-800 rounded-lg px-4 py-[14px] text-sm transition-all duration-300 flex items-center justify-between hover:border-cyan-500/50 focus:border-cyan-400 focus:outline-none shadow-sm"
              style={{ color: selected ? "#E0E0E0" : "rgba(224,224,224,.3)" }}
            >
              <span className="flex items-center gap-2.5">
                {selected ? (
                  <>
                    <span className="w-5 h-5 text-cyan-400" dangerouslySetInnerHTML={{ __html: serviceOptions.find((s) => s.label === selected)?.icon }} />
                    {selected}
                  </>
                ) : (
                  "Select a service"
              ...
              <svg width="12" height="8" viewBox="0 0 12 8" fill="none" className={`transition-transform duration-300 ${dropOpen ? "rotate-180" : ""}`}>
                <path d="M1 1.5L6 6.5L11 1.5" stroke="rgba(224,224,224,.35)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </button>

            <div className={`absolute top-full left-0 right-0 mt-2 rounded-xl border border-gray-800 overflow-hidden z-20 transition-all duration-300 ${dropOpen ? "opacity-100 translate-y-0 pointer-events-auto" : "opacity-0 -translate-y-2 pointer-events-none"}`}
              style={{ background: "rgba(17,17,17,0.98)", backdropFilter: "blur(16px)", boxShadow: "0 16px 48px rgba(0,0,0,.3)" }}
            >
              {serviceOptions.map((s) => (
                <button
                  key={s.label}
                  onClick={() => { setSelected(s.label); setDropOpen(false); }}
                  className={`w-full text-left px-4 py-3.5 flex items-center gap-3 text-sm transition-all duration-200 hover:bg-cyan-950/40 ${selected === s.label ? "bg-cyan-950/60 text-cyan-400" : "text-gray-400 hover:text-gray-200"}`}
                >
                  <span className="w-5 h-5 shrink-0" style={{ color: selected === s.label ? '#00F0FF' : 'rgba(224,224,224,.4)' }} dangerouslySetInnerHTML={{ __html: s.icon }} />
                  <span>{s.label}</span>
                  {selected === s.label && (
                    <svg className="ml-auto" width="14" height="14" viewBox="0 0 14 14" fill="none">
                      <path d="M2 7L5.5 10.5L12 3.5" stroke="#00F0FF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  )}
                </button>
              ))}
        ...
        <div className="reveal reveal-delay-3">
          <label className="block text-xs font-semibold tracking-wider uppercase text-gray-500 mb-2">Message</label>
          <textarea id="cMessage" placeholder="Tell me about your project…" className="form-textarea min-h-[140px]"></textarea>
        </div>

        {submitStatus === "success" && (
          <div className="reveal p-4 rounded-lg bg-emerald-950/30 border border-emerald-500/30 text-emerald-400 text-sm text-center" style={{ animation: "fadeUp .3s ease both" }}>
            Message sent successfully! I'll get back to you soon.
          </div>
        )}
        {submitStatus === "error" && (
          <div className="reveal p-4 rounded-lg bg-red-950/30 border border-red-500/30 text-red-400 text-sm text-center" style={{ animation: "fadeUp .3s ease both" }}>
            Failed to send message. Please try again or email me directly.
          </div>
        )}
        {submitStatus === "validation" && (
          <div className="reveal p-4 rounded-lg bg-amber-950/30 border border-amber-500/30 text-amber-400 text-sm text-center flex items-center justify-center gap-2" style={{ animation: "fadeUp .3s ease both" }}>
        ...
        <button
          ...
          className="reveal reveal-delay-4 w-full py-4 bg-gradient-to-r from-cyan-600 to-cyan-400 text-[#0A0A0A] font-semibold rounded-xl text-sm tracking-wide hover:shadow-[0_6px_28px_rgba(0,240,255,.3)] hover:-translate-y-0.5 transition-all duration-300 disabled:opacity-70 disabled:cursor-not-allowed disabled:hover:translate-y-0 disabled:hover:shadow-none flex items-center justify-center gap-2"
        >
          {isSubmitting ? (
            <>
