...
export default function Contact() {
  ...
  return (
    <section id="contact" className="px-12 py-24 max-md:px-6" style={{ background: "rgba(248,249,252,0.95)" }}>
      <h2 className="text-center text-xs font-semibold tracking-[.2em] uppercase text-slate-400 mb-2 reveal">Get In Touch</h2>
      <p className="text-center text-[clamp(1.6rem,3vw,2.2rem)] font-display font-bold mb-12 reveal reveal-delay-1 text-slate-800">
        Let's work together
      </p>
      ...
      <div className="max-w-[600px] mx-auto flex flex-col gap-5">
        ...
        <div className="reveal reveal-delay-3" ref={dropRef}>
          <label className="block text-xs font-semibold tracking-wider uppercase text-slate-400 mb-2">Service</label>
          <div className="relative">
            <button
              type="button"
              onClick={() => setDropOpen(!dropOpen)}
              className="w-full text-left bg-white border-[1.5px] border-slate-200 rounded-lg px-4 py-[14px] text-sm transition-all duration-300 flex items-center justify-between hover:border-indigo-300 focus:border-indigo-400 focus:outline-none shadow-sm"
              style={{ color: selected ? "#1e293b" : "rgba(30,41,59,.3)" }}
            >
              <span className="flex items-center gap-2.5">
                {selected ? (
                  <>
                    <span className="w-5 h-5 text-indigo-500" dangerouslySetInnerHTML={{ __html: serviceOptions.find((s) => s.label === selected)?.icon }} />
                    {selected}
                  </>
                ) : (
                  "Select a service"
              ...
              <svg width="12" height="8" viewBox="0 0 12 8" fill="none" className={`transition-transform duration-300 ${dropOpen ? "rotate-180" : ""}`}>
                <path d="M1 1.5L6 6.5L11 1.5" stroke="rgba(30,41,59,.35)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </button>

            <div className={`absolute top-full left-0 right-0 mt-2 rounded-xl border border-slate-200 overflow-hidden z-20 transition-all duration-300 ${dropOpen ? "opacity-100 translate-y-0 pointer-events-auto" : "opacity-0 -translate-y-2 pointer-events-none"}`}
              style={{ background: "rgba(255,255,255,0.98)", backdropFilter: "blur(16px)", boxShadow: "0 16px 48px rgba(0,0,0,.08)" }}
            >
              {serviceOptions.map((s) => (
                <button
                  key={s.label}
                  onClick={() => { setSelected(s.label); setDropOpen(false); }}
                  className={`w-full text-left px-4 py-3.5 flex items-center gap-3 text-sm transition-all duration-200 hover:bg-indigo-50 ${selected === s.label ? "bg-indigo-50/60 text-indigo-500" : "text-slate-600 hover:text-slate-800"}`}
                >
                  <span className="w-5 h-5 shrink-0" style={{ color: selected === s.label ? '#6366f1' : 'rgba(30,41,59,.4)' }} dangerouslySetInnerHTML={{ __html: s.icon }} />
                  <span>{s.label}</span>
                  {selected === s.label && (
        ...
        {submitStatus === "success" && (
          <div className="reveal p-4 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-600 text-sm text-center" style={{ animation: "fadeUp .3s ease both" }}>
            Message sent successfully! I'll get back to you soon.
          </div>
        )}
        {submitStatus === "error" && (
          <div className="reveal p-4 rounded-lg bg-red-50 border border-red-200 text-red-500 text-sm text-center" style={{ animation: "fadeUp .3s ease both" }}>
            Failed to send message. Please try again or email me directly.
          </div>
        )}
        {submitStatus === "validation" && (
          <div className="reveal p-4 rounded-lg bg-amber-50 border border-amber-200 text-amber-600 text-sm text-center flex items-center justify-center gap-2" style={{ animation: "fadeUp .3s ease both" }}>
